...
def format_to_parquet(src_file, dest_file):
    ...
    df = pd.read_csv(src_file, encoding = "ISO-8859-1", dtype={
                'VendorID': "Int64", 'passenger_count': "Int64", 'RatecodeID': "Int64", 
    ...
    df[['tpep_pickup_datetime','tpep_dropoff_datetime']] = df[['tpep_pickup_datetime','tpep_dropoff_datetime']].apply(pd.to_datetime, errors = 'coerce')
...
def download_parquetize_upload_bq_dag(
    ...
    with dag:
        with TaskGroup(group_id='yellow_data_processing_tasks', prefix_group_id=False) as yellow_data_processing_tasks:
        ...
        with TaskGroup('yellow_load_2_bq') as yellow_load_2_bq:
        ...
        yellow_data_processing_tasks >> yellow_load_2_bq


YELLOW_TAXI_URL_TEMPLATE = 'csv_backup/yellow_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.csv'
YELLOW_RENAME_FILE_TEMPLATE = "yellow_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.csv"
YELLOW_CSV_FILE_TEMPLATE = AIRFLOW_HOME + \
    "/data/yellow_tripdata_{{ execution_date.strftime(\'%Y-%m\') }}.csv"
YELLOW_PARQUET_FILE_TEMPLATE = AIRFLOW_HOME + \
...
YELLOW_DATASET = "tripdata"
YELLOW_COLOUR_RANGE = {'yellow': ('tpep_pickup_datetime', 'False')}
YELLOW_INPUT_PART = "raw"
YELLOW_INPUT_FILETYPE = "parquet"


yellow_etl_dag = DAG(
    dag_id="yellow_etl_dag",
...
download_parquetize_upload_bq_dag(
    ...
    rename_template=YELLOW_RENAME_FILE_TEMPLATE,
    ...
    DATASET=YELLOW_DATASET,
    COLOUR_RANGE=YELLOW_COLOUR_RANGE,
    INPUT_PART=YELLOW_INPUT_PART,
    INPUT_FILETYPE=YELLOW_INPUT_FILETYPE
)
